...
import { shopifyApp, ApiVersion, AppDistribution, BillingInterval, DeliveryMethod } from "@shopify/shopify-app-remix/server";
import { PrismaSessionStorage } from "@shopify/shopify-app-session-storage-prisma";
import prisma from "./db.server";

const requiredEnv = ["SHOPIFY_API_KEY","SHOPIFY_API_SECRET","SHOPIFY_APP_URL","SCOPES"];
for (const k of requiredEnv) { if (!process.env[k]?.trim()) throw new Error(`Missing env var: ${k}`); }
...
export const shopify = shopifyApp({
  ...
  billing: { plans: [{ id: "Free", amount: 0, currencyCode: "USD", interval: BillingInterval.Every30Days, trialDays: 0 }] },
  webhooks: {
