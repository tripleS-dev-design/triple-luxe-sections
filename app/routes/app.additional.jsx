...
export default function Pricing() {
  ...
  const HEADER_HERO = {
    background:
      "linear-gradient(135deg, #0a0a0a 35%, #1a1a1a 60%, #2a2a2a 100%)",
  ...
  const TITLE = {
    ...
    letterSpacing: ".3px",
    background:
      "linear-gradient(90deg,#d6b35b,#f0df9b 50%,#d6b35b)",
    WebkitBackgroundClip: "text",
    WebkitTextFillColor: "transparent",
  };

  // ➜ responsive sans JS (évite tout problème SSR)
  const GRID = {
    display: "grid",
  ...
  const Plan = ({ title, price, period, highlight, cta, planHandle }) => {
    const href = `/billing/activate?plan=${encodeURIComponent(
      planHandle
    )}${qs ? `&${qs}` : ""}`;
    ...
    <div style={CONTAINER_STYLE}>
      <section style={HEADER_HERO}>
        ...
        <p style={{ margin: "6px 0 0 0", opacity: 0.9 }}>
          Choisissez votre plan — les deux incluent toutes les sections. Essai gratuit 14 jours.
        </p>
      </section>
      ...
      <section style={GRID}>
        <Plan
          ...
          planHandle={PLAN_HANDLES.monthly}
        />
        <Plan
          title="Annuel"
          price="39,99 $"
          ...
          planHandle={PLAN_HANDLES.annual}
          highlight
        />
      </section>
    </div>
